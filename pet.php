<?php
    ...
    $petID = $_GET['id'];
    $pet = new Pet($petID);
    // Stuur gebruiker terug als de huisdier niet van hem is of
                ...
                <div class="card-header">
                    <nav class="nav nav-tabs card-header-tabs" id="petNav" role="tablist">
                        <a class="nav-item nav-link active" id="nav-appointment-tab" data-toggle="tab"
                           href="#nav-appointment" role="tab" aria-controls="nav-appointment" aria-selected="true">
                            Afspraken
                        </a>
                        <a class="nav-item nav-link" id="nav-treatment-tab" data-toggle="tab" href="#nav-treatment"
                           role="tab" aria-controls="nav-treatment" aria-selected="false">
                            Behandelingen
                        </a>
                    </nav>
                </div>
                            ...
                            <div class="card">
                                        ...
                                        <div class="card">
                                            <div class="card-header" id="heading<?php echo $appointment['name'] ?>">
                                                <button class="btn btn-link" data-toggle="collapse"
                                                    data-target="#collapse<?php echo $appointment['appointmentID'] ?>"
                                                    aria-expanded="true"
                                                    aria-controls="collapse<?php echo $appointment['appointmentID'] ?>">
                                                    <?php if (date_format(
                                                            date_create($appointment['date']), "H") == 0) : ?>
                                                    Aanvraag voor <?php echo
                                                        date_format(date_create($appointment['date']), "j M")?>
                                                    <?php else : ?>
                                                    Ingepland op <?php echo date_format(
                                                            date_create($appointment['date']), "j M - H:i")?>
                                                    <?php endif; ?>
                                                </button>
                                            </div>
                                            <div id="collapse<?php echo $appointment['appointmentID'] ?>"
                                                 class="collapse"
                                                 aria-labelledby="heading<?php echo $appointment['appointmentID'] ?>"
                                                 data-parent="#accordion">
                                                <div class="card-body">
                                                    <h5><?php echo $appointment['name'] ?></h5>
                                                    <?php  if (!empty($appointment['description'])) : ?>
                                                        <p><?php echo $appointment['description']; ?></p><?php endif; ?>
                                                </div>
                                            </div>
                                        </div>
                                    <?php endforeach; ?>
                                ...
                                <?php endif; ?>
                                    <button type="button" data-toggle="modal"
                                            data-target="#addAppointment" class="btn btn-primary">
                                            ...
                                            <div class="card-header" id="heading<?php echo $appointment['name'] ?>">
                                                <button class="btn btn-link" data-toggle="collapse"
                                                    ...
                                                    <?php echo date_format(
                                                            date_create($appointment['date']), "j M Y") ?>
                                                </button>
                                            </div>
                                            <div id="collapse<?php echo $appointment['appointmentID'] ?>"
                                                 class="collapse"
                            ...
                            <ul class="list-group">
                                ...
                                <li class="list-group-item d-flex justify-content-between align-items-center">
                                    ...
                                    <span class="badge badge-secondary badge-pill">
                                        <?php echo date_format(date_create($treatment['date']), "j M Y") ?>
                                    </span>
                                </li>
                                <?php endforeach; ?>
                            </ul>
                ...
                <form id="addAppointmentForm">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="name" class="form-control" id="name"
                               placeholder="Geef een reden voor de afspraak" required>
                    </div>
                    <div class="form-group">
                        <label for="datepicker">Voorkeursdatum</label>
                        <input type="text" name="date" id="date" class="form-control datepicker-future"
                               placeholder="Geef een voorkeursdag voor de afspraak" readonly="readonly">
                    </div>
                    <?php if (!$userObj->getPhone()) : ?>
                    <div class="form-group">
                        <label for="name">Telefoonnummer</label>
                        <input type="text" name="phone" class="form-control" id="phone"
                               placeholder="Geef uw telefoonnummer" max="10" required>
                    </div>
                    <?php endif; ?>
                    <div class="form-group">
                        ...
                        <textarea  name="description" id="description" class="form-control" aria-label="With textarea"
                                   placeholder="Geef wat extra informatie over de reden van uw afspraak"></textarea>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-dismiss="modal">Annuleren</button>
                        <button type="button" name="submitAppointment" id="submitAppointment" class="btn btn-success">
                            Maak afspraak
                        </button>
                    </div>
                </form>
...
<script>
    ...
    // Een aparte constant voor de telefoonnummer
    ...
    form.submit.addEventListener('click', function () {
        ...
        // Een check of de gebruiker nog geen telefoonnummer heeft
        if (phone !== null)  {
            if (phone.value === "") {
